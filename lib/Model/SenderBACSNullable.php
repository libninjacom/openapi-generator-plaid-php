...
/**
 * SenderBACSNullable Class Doc Comment
 *
 * @category Class
 ...
 */
class SenderBACSNullable implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'SenderBACSNullable';
    ...
    public function setAccount($account)
    {
        if (!is_null($account) && (mb_strlen($account) > 10)) {
            throw new \InvalidArgumentException('invalid length for $account when calling SenderBACSNullable., must be smaller than or equal to 10.');
        }
        if (!is_null($account) && (mb_strlen($account) < 1)) {
            throw new \InvalidArgumentException('invalid length for $account when calling SenderBACSNullable., must be bigger than or equal to 1.');
        }
    ...
    public function setSortCode($sort_code)
    {
        if (!is_null($sort_code) && (mb_strlen($sort_code) > 6)) {
            throw new \InvalidArgumentException('invalid length for $sort_code when calling SenderBACSNullable., must be smaller than or equal to 6.');
        }
        if (!is_null($sort_code) && (mb_strlen($sort_code) < 6)) {
            throw new \InvalidArgumentException('invalid length for $sort_code when calling SenderBACSNullable., must be bigger than or equal to 6.');
        }
